...
public class FacturaController {
	...
	/*
	 * - Mapping donde se carga la vista = /cargar-productos/{1} donde 1 indica que
	 * es el producto 1. /cargar-productos lo hemos indicado en el
	 * templates/factura/js/autocomplete-productos.html , url=
	 * /factura/cargar-productos/" + request.term - Genera una salida (produces) de
	 * aplication/json - @PathVariable, recibe una variable String term.
	 * - @ResponseBody, transforma la salida en json y la guarda dentro del body de
	 * la respuesta.
	 */
	...
	/*
	 * 1.- Guardamos la factura. 2.- Vamos a recoger los 2 parámertros que se indica
	 * en plantilla-items-html -> name="item_id[]" y name="cantidad[]" y
	 */

	@PostMapping("/form")
	/*
	 * Tenemos que añadir los nombres de los inputs de plantilla-items.xhtml para
	 * pasarlos por @Requestparam Para validar los campos en el messager.propertis
	 * debemos añadir el @Valid. que valida de forma automatica, también necesitaos
	 * el BindingResult que permite comprobar si hubieron errores en la factura.
	 * Importamos el MOdel para pasar datos a la vista
	 */
	public String guardar(@Valid Factura factura, BindingResult result, Model model,
			@RequestParam(name = "item_id[]", required = false) Long[] itemId,
			@RequestParam(name = "cantidad[]", required = false) Integer[] cantidad, RedirectAttributes flashmessage,
		...
		/*
		 * Por ultimo tenemos que eliminar el Tbody de plantilla-items por que no se
		 * tiene que enviar de modo que debemos irnos al autocomplete
		 */
	}
	...
	@GetMapping(value = "/buscarfactura")
	public String buscarfactura(@Param(value ="descripcion") String descripcion,
								Model model) throws Exception {
		try {
			model.addAttribute("titulo", "Factura del cliente");
			model.addAttribute("facturas", clienteService.findByIdFactura(descripcion));
		} catch (Exception e) {
